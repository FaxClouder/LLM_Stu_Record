...
本工作区统一使用仓库根目录 `.env`。如还没有该文件，先从根目录 `.env.example` 复制:

```bash
cp ../../../.env.example ../../../.env
```

> PS: 本教程使用阿里百炼平台的模型。你需要在 [阿里百炼平台](https://bailian.console.aliyun.com/) 注册账号，并将获得的 API 密钥填入仓库根目录 `.env` 文件中的 `DASHSCOPE_API_KEY` 变量。
...
# 加载模型配置
# 请事先在仓库根目录 .env 中配置 DASHSCOPE_API_KEY
_ = load_dotenv()
```
